...
static inline double complex ξ(int n, double *t, double *t_0, double *ω_0, double *Δ, int *permutation) {
...
double β(int n, double *xx, void *user_data) {
	...
	for (int i = 0; i < data->n_terms; ++i) {
		...
		for (int j = 0; j < data->n_permutation; ++j)
			subterm += ξ(n, xx, data->t_0, data->ω_0, data->Δ, &data->permutations[i*data->n_permutation*n + n*j]);

		double complex coeff = data->coeffs_real[i] + data->coeffs_imag[i]*I;
